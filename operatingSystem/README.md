...
CPU는 여러 개의 프로세스를 동시에 실행할 수 없고, 항상 한 개의 프로세스 수행만이 가능하다. 다만, 운영체제의 멀티 태스킹과 스케줄링 기법으로 여러 개의 프로세스가 동시에 실행되는 것처럼 보이게 해 주는 것이다.

#### 프로세스 상태

1. new : 프로세스가 생성되는 중
2. running : CPU에서 명령이 실행되는 중
3. waiting : 프로세스가 어떤 이벤트(입출력 완료, signal 수신)가 발생하기를 기다리는 중
4. ready : 프로세스가 CPU에 할당되어 실행되기를 기다리는 중
5. terminated : 프로세스 실행 종료

#### 운영체제 스케줄러

여러 개의 프로그램을 동시에 실행되는 것처럼 보이게 하기 위해 어떤 규칙을 부여하는 것.

#### 프로세스 상태 변화

1. new -> ready -> running

프로세스가 생성되면 Ready 상태가 되고, 여러 개의 프로세스들은 대부분 Ready 상태에 있고, 운영체제의 스케줄링 기법에 따라 그 중 한 개만 Running으로 바뀔 수 있다. 이때 Running 상태가 되면 실제 CPU를 이용해 하고자 하는 일을 수행한다.

2. running -> waiting -> ready

Running 상태에 있는 프로세스는 계속 실행되는 것이 아니라 어떤 조건이 만족되면 상태가 변경된다. 예를 들어 scanf, read, printf 등 외부 장치에 대한 입출력을 요구했을 때이다. 이 작업은 굉장히 느리기 때문에 CPU는 무작정 기다리는 것이 아니라 상태를 Waiting 상태로 변경하게 된다. Waiting이 끝나는 시점은 I/O 이벤트 완료 인터럽트가 발생될 때이다. 이 인터럽트가 발생되면 다시 Ready 상태가 되어 다시 스케줄러에 의한 실행을 기다리게 된다.
...
1ms ~ 10ms마다 타이머 인터럽트가 발생하여 운영체제는 현재 실행 중인 프로세스를 강제로 중지를 시킨 뒤 Ready 상태로 빼 둔다.
그리고 Ready 상태에 있는 다른 프로세스를 실행시킨다. 이 과정은 매우 빠르게 상태 변화를 일으키며 반복되기 때문에 마치 여러 개의 프로세스가 한 개의 CPU를 가지고 동시에 실행되는 것처럼 보이게 된다.

이를 통해 운영체제는 멀티 태스킹 멀티 프로세싱을 달성하게 된다.
